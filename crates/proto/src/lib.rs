...
pub mod iocompat {
    ...
    pub struct TokioAsyncIoBridge<T>(pub T);

    impl<R: AsyncRead + Unpin> TokioAsyncRead for TokioAsyncIoBridge<R> {
        fn poll_read(
            self: Pin<&mut Self>,
            cx: &mut Context<'_>,
            buf: &mut ReadBuf<'_>,
    ...
    impl<W: AsyncWrite + Unpin> TokioAsyncWrite for TokioAsyncIoBridge<W> {
        fn poll_write(
            self: Pin<&mut Self>,
            cx: &mut Context<'_>,
            buf: &[u8],
